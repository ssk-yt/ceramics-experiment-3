...
for _, row in si_std_df.iterrows():
    ...
    window = df[(df['2theta'] >= theo_2theta - 0.5) & (df['2theta'] <= theo_2theta + 0.5)]
    if not window.empty:
        # ウィンドウ内でピーク検出
...
for _, row in ref_strong.iterrows():
    ...
    if d_inv2 <= 0: continue
    d = np.sqrt(1/d_inv2)
    
    if lam / (2*d) > 1: continue
    theo_th = 2 * np.degrees(np.arcsin(lam / (2*d)))
    
    # 近くの実験ピークを探す
    window = df[(df['2theta_corr'] >= theo_th - 0.5) & (df['2theta_corr'] <= theo_th + 0.5)]
    if not window.empty:
...
for p in correction_points:
    search_th = p['theo']
    window = df[(df['2theta_corr'] >= search_th - 0.3) & (df['2theta_corr'] <= search_th + 0.3)]
    if not window.empty:
...
for l in unique_labels:
    plt.text(l['x'], l['y'] + 500, l['label'], 
             rotation=90, ha='center', va='bottom', fontsize=8, 
             color=l['color']) # ★変更点: 指定した色でテキストを描画
